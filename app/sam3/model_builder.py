...
def _load_checkpoint(model, checkpoint_path):
    ...
    # .pt keys use "detector." prefix; .safetensors use "detector_model." prefix
    def _strip_detector(k):
        if k.startswith("detector_model."):
            return k[len("detector_model."):]
        if k.startswith("detector."):
            return k[len("detector."):]
        return k.replace("detector.", "")  # legacy fallback

    sam3_image_ckpt = {
        _strip_detector(k): v for k, v in ckpt.items() if "detector" in k
    }
    if model.inst_interactive_predictor is not None:
        def _strip_tracker(k):
            if k.startswith("tracker_model."):
                return "inst_interactive_predictor.model." + k[len("tracker_model."):]
            if k.startswith("tracker_neck."):
                return "inst_interactive_predictor.model.neck." + k[len("tracker_neck."):]
            return k.replace("tracker.", "inst_interactive_predictor.model.")
        sam3_image_ckpt.update(
            {
                _strip_tracker(k): v
                for k, v in ckpt.items()
                if k.startswith("tracker") or "tracker" in k
            }
        )
